...
fetch('/getPoints')
  ...
  .then(data => {
    ...
    if ("geolocation" in navigator) {
      let customIconGeoLoc = {
        iconUrl: "./icons/geoLoc.png",
        iconSize: [40, 40]
      }

      let myIconGeoLoc = L.icon(customIconGeoLoc);

      let iconOptionsGeoLoc = {
        icon: myIconGeoLoc
      }
      navigator.geolocation.getCurrentPosition(function (position) {
        var userLat = position.coords.latitude;
        ...
        var userMarker = L.marker([userLat, userLng], iconOptionsGeoLoc).addTo(carte);
    ...
    fetch('/getUrgence')
      .then(response => response.json())
      .then(data => {
        console.log('Résultats de la requête urgence SELECT :', data);

        let customIconUrgence = {
          iconUrl: "./icons/urgence.png",
          iconSize: [40, 40]
        }
    
        let myIconUrgence = L.icon(customIconUrgence);
    
        let iconOptionsUrgence = {
          icon: myIconUrgence
        }
    
        for (let urgence of data) {
          var marqueur = L.marker([urgence.latitude, urgence.longitude], iconOptionsUrgence);
          marqueur.bindPopup("<p><strong>Needs </strong>(" + urgence.type + " on " + urgence.date.slice(0, -14) + ")</p>");
          marqueurs.addLayer(marqueur); //On ajoute le marqueur au groupe
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById("urgenceButton").addEventListener("click", () => {
    ...
    document.getElementById('latitudeU').value = lat;
    document.getElementById('longitudeU').value = lng;
  });
